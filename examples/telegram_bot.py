...
class TelegramCompanion:
    ...
    def __init__(
        ...
        self.last_message_sent: Optional[datetime] = None
        self.message_count = 0
        
        logger.info(f"🤖 TelegramCompanion initialized")
        logger.info(f"   Chat ID: {chat_id}")
        logger.info(f"   Check interval: {check_interval} min")
        logger.info(f"   Quiet hours: {quiet_hours[0]:02d}:00 - {quiet_hours[1]:02d}:00")
    
    def is_quiet_hour(self, now: Optional[datetime] = None) -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="💘 revive-my-lover Telegram Bot"
    )
    parser.add_argument(
        "--token",
        required=True,
    ...
    parser.add_argument(
        "--chat-id",
        required=True,
        help="Target chat ID (get from @userinfobot)"
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Quiet hours start (default: 0)"
    )
    parser.add_argument(
        "--quiet-end",
        type=int,
        default=8,
        help="Quiet hours end (default: 8)"
    )
