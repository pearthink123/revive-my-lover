...
from .core import State, TRANSITIONS
...
class BayesianLearner:
    ...
    def __post_init__(self):
        ...
        self._transition_counts: Dict[State, Dict[State, int]] = defaultdict(lambda: defaultdict(int))
        
        # Observation values: state -> obs_key -> list of values
        self._observation_values: Dict[State, Dict[str, List[float]]] = defaultdict(lambda: defaultdict(list))
        ...
        # Learned parameters
        self._learned_transitions: Optional[Dict[State, Dict[State, float]]] = None
        self._learned_likelihoods: Optional[Dict[State, Dict[str, tuple]]] = None
        self._learned_temporal: Optional[Dict[State, Dict[int, float]]] = None
        ...
        for from_state in State:
            ...
            if most_common and max_count > 0:
                ...
                if probability > 0.3:  # Only report significant patterns
                    patterns.append(
                        f"{from_state.value} → {most_common.value} ({probability:.0%})"
                    )
